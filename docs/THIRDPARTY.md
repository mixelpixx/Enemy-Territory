...
| 2026-06-12 | src/renderer2/glsl/lightMapping_vp.glsl | ET:Legacy v2.84.0 @ 764ffc00a953e59aaf435272d004c49a89710309 src/renderer2/glsl/lightMapping_vp.glsl | GPL-3.0 | **mechanical edit (1 uniform decl)** | R2-5 Task 4 — added the missing `uniform float u_DepthScale;` declaration (guarded by `#if defined(USE_PARALLAX_MAPPING)`, inside the existing `USE_NORMAL_MAPPING` uniform block). Latent upstream bug: the parallax-mapping vertex code (`var_S = ... * u_DepthScale`) references `u_DepthScale` but this shader never declared the uniform, so it ERR_DROPs with `'u_DepthScale' : undeclared identifier` (dumping debug/lightMapping_vertex.debug) whenever `r_parallaxMapping` is enabled on lightmapped world surfaces. Declaration copied verbatim from the sibling parallax-capable vertex shaders (forwardLighting_vp/liquid_vp/vertexLighting_DBS_entity_vp/vertexLighting_DBS_world_vp), all of which declare it the same way. The new uniform exists only in the USE_PARALLAX_MAPPING permutation, so default (parallax-off) rendering is unchanged. Second source edit to the otherwise-verbatim renderer2 tree; clearly commented in-file with the RM/R2-5 marker |
| 2026-06-11 | src/renderer2/etl/tr_glsl.c | ET:Legacy v2.84.0 @ 764ffc00a953e59aaf435272d004c49a89710309 src/renderer2/tr_glsl.c | GPL-3.0 | **mechanical edit (1 line)** | R2-3 Task 4 — added a `&& glState.currentVBO` guard to the ATTR_COLOR pointer-setup in `GLSL_SetUniform_ColorModulate()` so it no longer dereferences a NULL `glState.currentVBO` (`->ofsColors`, reads NULL+0x64). This is a latent upstream crash: it fires when ColorModulate runs before any VBO is bound — i.e. `RE_StretchRaw()` on the very first 2D draw, which for ET-RM is the intro RoQ cinematic at boot. The guard mirrors the project's own identical NULL-VBO guard already present in `GLSL_VertexAttribPointers()` ("no current VBO bound") in the same file; the skipped pointer-setup is re-established by the subsequent Tess_UpdateVBOs→R_BindVBO. First source edit to the otherwise-verbatim renderer2 tree; clearly commented in-file with the RM/R2-3 marker |
| 2026-06-19 | src/renderer2/etl/{tr_bsp,tr_image,tr_init,tr_shade}.c, src/renderer2/etl/{tr_local,tr_glsldef}.h, src/renderer2/gldef/default.gldef, src/renderer2/glsl/lightMapping_fp.glsl | ET:Legacy v2.84.0 @ 764ffc00a953e59aaf435272d004c49a89710309 (same upstream files) | GPL-3.0 | **edit (re-enable + complete the deluxe-mapping feature)** | R2-6 Task 1b — deluxe (per-pixel directional lightmap) lighting, which upstream stubbed out wholesale ("there is no code that does anything with deluxemaps, we disable it"). Re-enabled and completed: (1) `trGlobals_t.worldDeluxeMapping` + `.deluxemaps` growlist (tr_local.h), growlist init/destroy (tr_image.c); (2) worldspawn detection of `deluxeMapping 1` / q3map2 `_q3map2_cmdline -deluxe`, the interleaved (even=lightmap/odd=deluxemap) internal & external lightmap loaders, the surface lightmapNum `/2` remap, and per-map flag reset (tr_bsp.c); (3) `UNIFORM_DELUXEMAP`/`u_DeluxeMap`, `UNIFORM_B_SHOW_DELUXEMAP`, `TEX_DELUXE` X-macro defs (tr_glsldef.h), `r_showDeluxeMaps` debug cvar (tr_init.c, CVAR_CHEAT); (4) `BindDeluxeMap` consumer un-commented + wired into `Render_lightMapping` with `use_deluxeMapping = normalMapping && worldDeluxeMapping && deluxemaps>0` (tr_shade.c); (5) **the lighting itself**, which never existed: lightMapping_fp.glsl now decodes the per-texel deluxe direction (tangent→world) and lights each pixel with it instead of the single sun dir, plus an `r_showDeluxeMaps` debug visualization. KEY FIX: added `USE_DELUXE_MAPPING` to the lightMapping shader's `macros` line in default.gldef — without it the macro was silently dropped and the permutation selector misaligned (rendered garbage). Inert on stock content: the 2003 retail BSPs carry neither deluxe key, so `worldDeluxeMapping` stays false and the gl1-faithful/stock-map output is unchanged (verified: oasis under gl2 shows no deluxe, renders normally). All edits live only inside etrm_renderer2.dll; gl1 and the engine/sim are untouched. Clearly commented in-file with RM/R2-6 markers |
