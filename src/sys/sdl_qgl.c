...
void QGL_Shutdown( void ) {
	...
	// RM/SDL: SDL owns the GL library/context; nothing to FreeLibrary here.
...
qboolean QGL_Init( const char *dllname ) {
	// RM/SDL: the GL driver is already loaded and a context is current
	// (SDL_GL_CreateContext was called in GLimp_Init). We do NOT LoadLibrary
	// the GL driver here; SDL owns the context. dllname is unused.
	(void)dllname;

	ri.Printf( PRINT_ALL, "...initializing QGL (SDL proc loader)\n" );
	...
	// RM/SDL: the WGL-specific entry points (qwgl*) are gone. SDL owns the
	// context, buffer swap, pixel format and gamma. Anything that previously
	// resolved through qwglGetProcAddress is resolved by SDL_GL_GetProcAddress
	// (via GLW_InitExtensions in sdl_glimp.c) instead.
